    ...
    fun upgradeTest() {
        try {
            ...
            if (GnssTestData.versionS.value != releasVer) {//需要升级
                ...
                while (!ping) {
                    Thread.sleep(1000)
                    if (releasVer == GnssTestData.versionS.value ) {
                        putLogInfo("正式版本更换完成!当前软件版本:${GnssTestData.versionS.value}")
                        showAlert("成功")
                        break
                    }
                ...
                putLogInfo("查询版本不一致!当前软件版本:${GnssTestData.versionS.value}")
                showAlert("查询版本不一致")
